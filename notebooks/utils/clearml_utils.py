"""Utility helpers for working with ClearML from training notebooks."""
...
def start_experiment(
    ...
    *,
    ...
    task = task_cls.init(project_name=project_name, task_name=task_name, tags=resolved_tags or None)
...
def upload_model_artifact(
    ...
    *,
    ...
    if not _resolve_enabled(enabled) or not run.enabled or run.task is None:
        return
    run.task.upload_artifact(name=alias, artifact_object=str(path))
